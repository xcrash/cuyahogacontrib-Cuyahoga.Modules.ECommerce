...
namespace Cuyahoga.Modules.ECommerce.Util {
	...
	public sealed class WebHelper {
        ...
        public const string CONFIG_KEY_MAX_COLUMNS_PER_ROW = "MaxColumsPerRow";
        ...
        public static string GetLargeImagePathWeb() {
            string mappedAppPath = HttpContext.Current.Request.MapPath(GetAppPath());
            return GetAppPath() + GetImageWorkingDirectory().Substring(mappedAppPath.Length).Replace("\\", "/") + "web/Large/";
        }

        public static string GetLargeImageUrl(string filePath) {
            if (filePath == null) return "";
            string mappedAppPath = GetImageWorkingDirectory() + "web\\";
            return filePath.Substring(mappedAppPath.Length).Replace("\\", "/Large/");
        }

        public static int GetMaxCoulmnsPerRow() {
            return Convert.ToInt32(ConfigurationManager.AppSettings[CONFIG_KEY_MAX_COLUMNS_PER_ROW]);
        }

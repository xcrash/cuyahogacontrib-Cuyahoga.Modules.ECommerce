...
namespace Cuyahoga.Modules.ECommerce.Util {
	...
	public class PaymentHelper {
		...
		public void PopulatePayment(IElectronicPayment payment, IBasket order, ITextTranslator translator) {
            ...
            payment.UserInfo.UserName = userDetails.FirstName + " " + userDetails.LastName;
            payment.UserInfo.EmailAddress = userDetails.EmailAddress;
            payment.UserInfo.TelephoneNumber = userDetails.TelephoneNumber;

			IAddress invoiceAddress = basket.OrderHeader.InvoiceAddress; 

			payment.UserInfo.Address1 = invoiceAddress.AddressLine1;

			if (invoiceAddress.AddressLine3 != null && invoiceAddress.AddressLine3.Length > 0) {
				payment.UserInfo.Address2 = invoiceAddress.AddressLine2 + ", " + invoiceAddress.AddressLine3;
			} else {
				payment.UserInfo.Address2 = invoiceAddress.AddressLine2;
			}

			payment.UserInfo.City = invoiceAddress.City;
			payment.UserInfo.State = invoiceAddress.Region;
			payment.UserInfo.PostCode = invoiceAddress.Postcode;
			
			ITextTranslator ccTrans = TranslatorUtils.GetTextTranslator(typeof(CountryCode), translator.CultureCode);
			payment.UserInfo.CountryName = invoiceAddress.CountryCode;
		}

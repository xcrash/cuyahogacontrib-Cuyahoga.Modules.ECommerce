			...
			switch (paymentType) {
				...
				case PaymentRequestTypes.ReservePayment:
					...
					try {
						...
						payment.UserInfo.PostCode = postBackData["postcode"];
						payment.UserInfo.EmailAddress = postBackData["email"];
						payment.UserInfo.Address1 = postBackData["address"];
						payment.UserInfo.CountryName = postBackData["countryString"];
						payment.UserInfo.TelephoneNumber = postBackData["tel"];
						payment.UserInfo.FaxNumber = postBackData["fax"];
					} catch (Exception f) {
						Logger.Error(f);
					}
			...
			switch (paymentType) {
				...
				case PaymentRequestTypes.ReservePayment:
					...
					if (payment.UserInfo != null) {
						...
						html.Append("<input type=\"hidden\" name=\"postcode\" value=\"" + payment.UserInfo.PostCode + "\">\n");
						html.Append("<input type=\"hidden\" name=\"country\" value=\"" + payment.UserInfo.CountryName + "\">\n");
						html.Append("<input type=\"hidden\" name=\"tel\" value=\"" + payment.UserInfo.TelephoneNumber + "\">\n");

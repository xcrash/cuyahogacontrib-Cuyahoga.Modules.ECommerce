...
namespace Cuyahoga.Modules.ECommerce.Service.PaymentProvider.Streamline {
	...
	public class StreamlineRedirectPaymentProvider : AbstractWebFormPaymentProvider {
		...
		public string CreateXML(IElectronicPayment payment) {
            ...
            if (payment.UserInfo != null && payment.UserInfo.UserDetails.EmailAddress != null && payment.UserInfo.UserDetails.EmailAddress.Length > 0) {
				XmlElement shopper = XMLUtils.AppendElement(order, "shopper");
				XMLUtils.AppendElement(shopper, "shopperEmailAddress", payment.UserInfo.UserDetails.EmailAddress);
			}
		...
		private void AddAddressInfo(IElectronicPayment payment, XmlElement order) {
            ...
            if (payment.UserInfo != null && payment.UserInfo.UserAddress.AddressLine1 != null && payment.UserInfo.UserAddress.AddressLine1.Length > 0) {
				...
				Match match = streetParser.Match(payment.UserInfo.UserAddress.AddressLine1);
				...
				if (match.Groups.Count == 4 && match.Groups[3].Value != null&& match.Groups[3].Value.Length > 0) {
					...
					XMLUtils.AppendElement(address, "street", payment.UserInfo.UserAddress.AddressLine1);
				}

				XMLUtils.AppendElement(address, "postalCode", payment.UserInfo.UserAddress.Postcode);
                XMLUtils.AppendElement(address, "city", payment.UserInfo.UserAddress.City);
                XMLUtils.AppendElement(address, "state", payment.UserInfo.UserAddress.Region);
                XMLUtils.AppendElement(address, "countryCode", payment.UserInfo.UserAddress.CountryCode);

				XMLUtils.AppendElement(address, "telephoneNumber", payment.UserInfo.UserDetails.TelephoneNumber);
			}
		}

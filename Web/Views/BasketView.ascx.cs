namespace Cuyahoga.Modules.ECommerce.Web.Views {
    ...
    public class BasketView : BasketCommon {
        ...
        private string GetProductImageUrl(string itemCode) {

            Cuyahoga.Modules.ECommerce.Domain.Catalogue.Image image = CatMod.CatalogueViewer.GetImageByItemCode(CatMod.Section.Node.Site.Id, CatMod.Section.Node.Culture, itemCode);
            if (image != null && image.Url != null && image.Url != String.Empty) {
                return image.Url;
            }

            return ""; // Should maybe return a default image.
        }
        ...
        protected virtual void DecorateUpdateButtons(int i) {
            ...
            //Button btnUpdateQuantity = (Button) rBasket.Items[i].FindControl("btnUpdateQuantity");
            //btnUpdateQuantity.AlternateText = GetText("update quantity");

            //Button btnRemoveItem = (Button) rBasket.Items[i].FindControl("btnRemoveItem");

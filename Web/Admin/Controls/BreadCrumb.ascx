...
<itemtemplate><%#(Container.ItemIndex == 0) ? ("") : (NodeSeperator + ((ShowCatItemLink(Container) ? ("<a href=\"CatalogueBrowser.aspx?NodeId=19&SectionId=41&cat=" + ((TrailItem)Container.DataItem).NodeID + "\">") : "") + ((TrailItem)Container.DataItem).Name + ((ShowCatItemLink(Container) ? "</a>" : ""))))%> </itemtemplate>
